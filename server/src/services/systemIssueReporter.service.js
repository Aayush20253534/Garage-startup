...
const getActorContext = async (req, payload = {}) => {
  ...
  return { actorType, userId, garageId };
};

const buildIssueData = async ({ req = null, error = null, payload = {} }) => {
  const message =
    truncate(payload.message || error?.message || "Unknown application error", MAX_MESSAGE_LENGTH) ||
    "Unknown application error";
  const errorName = truncate(payload.errorName || error?.name || "Error", 120);
  const source = payload.source === "BACKEND" ? "BACKEND" : "FRONTEND";
  const httpStatus = Number(payload.httpStatus || error?.statusCode || 0) || null;
  const severity = ["INFO", "WARNING", "ERROR", "CRITICAL"].includes(
    ...
    : httpStatus && httpStatus >= 500
      ? "ERROR"
      : "WARNING";
  const route = truncate(payload.route || req?.originalUrl || req?.path, 500);
  const endpoint = truncate(payload.endpoint || req?.originalUrl, 500);
  const method = truncate(payload.method || req?.method, 12)?.toUpperCase() || null;
  const component = truncate(payload.component, 180);
  const title =
    truncate(payload.title || `${errorName}: ${message}`, MAX_TITLE_LENGTH) ||
    "Application issue";
  ...
  const ipAddress = truncate(req?.ip || payload.ipAddress, 100);
  const environment = truncate(
    payload.environment || process.env.NODE_ENV || "development",
    60,
  );
  const release = truncate(payload.release || process.env.RENDER_GIT_COMMIT, 120);
  const stack = truncate(payload.stack || error?.stack, MAX_STACK_LENGTH);
...
const storeIssue = async (data) => {
  ...
  const shouldReopen = ["RESOLVED", "IGNORED"].includes(existing.status);
...
const captureIssue = async ({ req = null, error = null, payload = {} }) => {
  try {
    const data = await buildIssueData({ req, error, payload });
...
const captureRequestError = (error, req, context = {}) =>
  captureIssue({
    req,
    error,
    payload: {
      source: "BACKEND",
      severity: Number(context.statusCode || error?.statusCode || 500) >= 500
        ? "ERROR"
        : "WARNING",
      httpStatus: context.statusCode || error?.statusCode || 500,
      title: context.title || "Backend request failed",
      component: context.component || "Express error middleware",
      metadata: {
        params: req?.params,
        query: req?.query,
...
const captureBackgroundError = (error, context = {}) =>
  captureIssue({
    error,
    payload: {
      source: "BACKEND",
      ...
      title: context.title || "Background process failed",
      component: context.component || "Background worker",
