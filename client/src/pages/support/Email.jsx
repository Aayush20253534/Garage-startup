    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
        <section className="rounded-2xl border border-line bg-white p-5 shadow-soft">
          ...
          <form onSubmit={submitSearch} className="mt-4 flex gap-2"><div className="relative flex-1"><FiSearch className="absolute left-3 top-3.5 text-muted" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, email, or phone" className="h-11 w-full rounded-lg border border-line pl-10 pr-3 text-sm outline-none focus:border-ink" /></div><button className="h-11 rounded-lg bg-ink px-4 text-sm font-bold text-white">Search</button></form>
        ...
        <form onSubmit={submit} className="rounded-2xl border border-line bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <div className="flex justify-end"><button disabled={sending || !selectedUser} className="inline-flex h-11 items-center gap-2 rounded-lg bg-ink px-5 text-sm font-bold text-white disabled:opacity-50"><FiSend /> {sending ? "Sending..." : "Send email"}</button></div>
          </div>
        </form>
      </div>
