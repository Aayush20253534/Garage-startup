...
export default function HomepageBanners() {
  ...
  const busy = createMutation.isPending || updateMutation.isPending || reorderMutation.isPending || deleteMutation.isPending;
  ...
  const submit = async (event) => {
    ...
    if (!Number.isInteger(seconds) || seconds < 1 || seconds > 300) return setError("Duration must be between 1 and 300 seconds.");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5">
      <div className="rounded-2xl border border-line bg-white p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-muted">Add rotating hero images, choose how long each stays visible, reorder them, or deactivate them without deleting the image.</p>
          </div>
      ...
      <form onSubmit={submit} className="grid gap-4 rounded-2xl border border-line bg-white p-5 sm:grid-cols-[1fr_150px] sm:p-6">
        ...
        <label className="text-sm font-bold text-ink">Duration (seconds)<input required type="number" min="1" max="300" step="1" value={duration} onChange={(event) => setDuration(event.target.value)} className={`${fieldClass} mt-2`} /></label>
        <label className="text-sm font-bold text-ink sm:col-span-2">Banner image<input ref={fileInputRef} required type="file" accept="image/*" onChange={(event) => setImage(event.target.files?.[0] || null)} className="mt-2 block w-full rounded-lg border border-line p-2 text-sm" /><span className="mt-1 block text-xs font-normal text-muted">One image is responsively cropped on desktop and mobile. Maximum 8 MB.</span></label>
        <button disabled={busy} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand px-4 text-sm font-bold text-black hover:brightness-95 disabled:opacity-50 sm:col-span-2 sm:w-fit"><FiPlus /> {createMutation.isPending ? "Uploading…" : "Add banner"}</button>
      </form>
        ...
        {banners.map((banner, index) => (
          <article key={banner.id} className={`grid gap-4 rounded-2xl border bg-white p-4 sm:grid-cols-[180px_1fr_auto] sm:items-center ${banner.isActive ? "border-line" : "border-dashed border-gray-300 opacity-70"}`}>
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted">Shown for {banner.duration} second{banner.duration === 1 ? "" : "s"}</p>
