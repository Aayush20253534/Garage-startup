...
export default function SubAdminForgotPassword() {
  ...
  const requestOtp = async (event) => {
    ...
    try { await adminApi.requestSubAdminPasswordReset(email.trim()); setStep("RESET"); setSuccess("If this active sub-admin email exists, a reset OTP has been sent."); }
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8">
    <section className="mx-auto w-full max-w-lg rounded-2xl border border-line bg-white p-5 shadow-soft sm:p-7">
      ...
      <div className="mt-7 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-ink text-white"><FiKey /></span><div><p className="text-xs font-bold uppercase tracking-wider text-muted">Sub-admin recovery</p><h1 className="text-2xl font-extrabold text-ink">Reset password</h1></div></div>
